...
MibRequestConfirm_t mib;
  ...
  LoRaMacMibGetRequestConfirm(&mib);
    ...
    {  
#ifdef LORAMAC_CLASSB_ENABLED
       if (Ctx.NvmCtx->MulticastSlotState != PINGSLOT_STATE_RX) /*unicast mode*/
       {
    ...
    {  
      AT_PRINTF("+EVT:RXC, RSSI %u, SNR %d\r\n", lora_config_rssi_get(),lora_config_snr_get());
    }
    else
    {
      AT_PRINTF("+EVT:RX3, RSSI %u, SNR %d\r\n", lora_config_rssi_get(),lora_config_snr_get());
    } 
}
...
ATEerror_t at_DeviceClass_get(const char *param)
{
  ...
  CHECK_STATUS(status);
    ...
    {  
      ...
      {  
         if (Ctx.NvmCtx->BeaconCtx.Ctrl.AcquisitionPending == 1) /*Beacon_Searching on Class B request*/
         {  
           AT_PRINTF("%s\r\n", "B,S0" );
         }
         else
         {  
           if ((Ctx.NvmCtx->BeaconCtx.Ctrl.BeaconAcquired == 1) && Ctx.NvmCtx->BeaconCtx.Ctrl.BeaconMode == 1) /*Beacon locked */
           {  
             AT_PRINTF("%s\r\n", "B,S1");
           }
           else
...
uint16_t PingPeriod;
  ...
  if ((tiny_sscanf(param, "%hhu", &PinSlotPeriodicity) > 7) && (tiny_sscanf(param, "%hhu", &PinSlotPeriodicity) < 0)  )
  {
    return AT_PARAM_ERROR;
  }
  
  PingNb = 128 / (1 << PinSlotPeriodicity);
  PingPeriod =  4096 / PingNb;

  Ctx.NvmCtx->PingSlotCtx.PingPeriod =  PingPeriod;
...
MibRequestConfirm_t mib;
    ...
    {  
      if ((Ctx.NvmCtx->BeaconCtx.Ctrl.BeaconAcquired == 1) && Ctx.NvmCtx->BeaconCtx.Ctrl.BeaconMode == 1) /*Beacon locked */
      {  
          AT_PRINTF("BTIME:%d\r\n", Ctx.NvmCtx->BeaconCtx.BeaconTime);
          return AT_OK; 
      }
    }  
    else
...
MibRequestConfirm_t mib;
    ...
    if( mib.Param.Class == CLASS_B)
    {  
      if (Ctx.NvmCtx->BeaconState == BEACON_STATE_LOCKED) /*Beacon locked on Gateway*/
      {  
